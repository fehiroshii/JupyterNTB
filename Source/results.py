...
def show_resultstable(units,conversion,weight,avg_lst):
    ...
    df1 = pd.DataFrame([beats_per_min + ' bpm',
                        ...
                        avg_cardiac_out + ' ' + units[2] + '/min/kg',
                        avg_ejection_fraction + '%'],
                    index = pd.Index(["BPM:","Ejection Volume:","Cardiac Output:","Ejection Fraction:"]),
...
def get_result_lst(min_list,max_list,time_axis,volume_axis,width_axis,height_axis):
    ...
    return freq,[min_volume,min_width,min_height],[max_volume,max_width,max_height],[avg_freq,avg_max_volume,avg_min_volume]

    
# Show all results widgets 
def show_results(weight,time,minor_axis,major_axis,area,rev_area,ellipse_area,ellipse_vol,rev_vol,const_vol,trgg):
    ...
    menu_ch_volume = widgets.Dropdown(
        options=[('Prolate Spheroid',1), ('Revolution Solid',2), ('Ellipse Major Axis Const',3)],
        value= 1,
    ...
    with outt3:
        display(show_resultstable(["_","_",'nl'],[1,1,1e-6],weight,avg_list))
    with outt4:
    ...
    def on_button_clicked (a):
        ...
        multiplier = [1,1,1]
        units = [menu_lenght.value,menu_area.value,menu_volume.value]
        ...
        frq,min_list,max_list,avg_list = get_result_lst(min_points,max_points,time,volume,minor_axis,major_axis)
